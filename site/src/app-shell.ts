...
class AccountShell {
  ...
  private renderDeviceActivation(): string {
    ...
    const approvalControls =
      eligibleContexts.length > 0
        ? `<label>Authorize for<select id="activation-organization">${contexts}</select></label>
  ...
  private renderProductShell(): string {
    ...
    return `<div class="rc-cloud-shell">
      ...
      <aside class="rc-cloud-rail">
        ...
        <div class="rc-cloud-context-group"><label class="rc-cloud-context"><span>${this.context?.kind === "personal" ? "Personal" : this.context?.plan === "enterprise" ? "Enterprise" : "Organization"}</span><select id="context-selector">${contextOptions}</select></label><button class="rc-cloud-context-create" type="button" data-action="open-organization-dialog">New organization</button></div>
        ${primary}
  ...
  private renderGrantForm(): string {
    ...
    return `<form class="rc-cloud-form" data-form="node-grant"><label>Member<select name="principal_user_id">${eligibleMembers.map((member) => `<option value="${escapeHtml(member.userId)}">${escapeHtml(member.name)} · ${escapeHtml(member.role)}</option>`).join("")}</select></label><label>Node<select name="host_id">${this.hosts.map((host) => `<option value="${escapeHtml(host.id)}">${escapeHtml(host.name)}</option>`).join("")}</select></label><label>Permission<select name="permission"><option value="use">Use terminal</option><option value="manage">Manage Node</option></select></label><button class="rc-cloud-button rc-cloud-button--primary" type="submit" ${this.busy ? "disabled" : ""}>${this.busy === "grant:create" ? "Saving…" : "Grant or update access"}</button><p class="rc-cloud-fineprint">Use opens terminal sessions. Manage also permits Node administration. View-only is not offered because it cannot open the terminal.</p></form>`;
  }
