...
describe("hosted terminal build contract", () => {
  ...
  test(
    "builds staging under a production hold before preserving the real PWA through the site build",
    { timeout: 120_000 },
    () => {
      const siteDirectory = process.cwd();
      execFileSync(process.execPath, ["scripts/build.mjs", "--target=staging"], {
        cwd: siteDirectory,
        env: {
          ...process.env,
          WORKERS_CI: "1",
