...
describe("GCP Cloudflare Tunnel deployment profile", () => {
  ...
  test("fails deployment verification when public HTTP or response policy is unsafe", () => {
    ...
    expect(caddy).toContain("@internalManagement path /internal /internal/*");
    expect(caddy).toContain("@rootAccountManagement path /v1/accounts /v1/accounts/*");
    expect(caddy).toMatch(/@rootRouteCollection \{\s+method GET HEAD POST\s+path \/v1\/routes\s+\}/);
    expect(caddy).toMatch(/@rootRouteDelete \{\s+method DELETE\s+path_regexp rootRouteResource/);
    expect(caddy).not.toContain("path /v1/account /v1/account/*");
    expect(caddy).not.toContain("path /v1/routes/*");
    ...
    expect(publicVerification).toContain("/internal/v1/accounts/rra_publicprobe00001/status");
    expect(publicVerification).toContain("/v1/accounts");
    expect(publicVerification).toContain("root_route_delete_code");
    expect(publicVerification).toContain('test "$management_code" = 404');
  ...
  test("proves public WebSocket upgrades with a transient route and guaranteed cleanup", () => {
    ...
    expect(wrapper).toContain('--network "container:${tunnel_container}"');
    ...
    expect(wrapper).toContain("ROAMCODE_RELAY_INTERNAL_ORIGIN=http://${relay_internal_address}:4281");
    expect(wrapper).toContain("relay_internal_address=$(docker inspect");
    expect(wrapper).not.toContain("ROAMCODE_RELAY_ROOT_TOKEN=");
    expect(smoke).toContain("isPrivateRelayOrigin(relayInternalOrigin)");
    expect(smoke).toContain('relayRequest(relayInternalOrigin, accountPath, "PUT", rootToken');
    ...
    expect(smoke).toContain("let accountCreated = false");
    expect(smoke).toContain("if (accountCreated)");
    expect(smoke).toContain("total > 64 * 1024");
    expect(smoke).toContain('relayRequest(relayInternalOrigin, accountPath, "DELETE", rootToken');
