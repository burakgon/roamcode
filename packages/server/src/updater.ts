...
export function renderUpdaterScript(opts: RenderUpdaterScriptOptions): string {
  ...
  const body = [
    ...
    "# UNTRACKED files are deliberately ignored (-uno): an ff-only pull can't harm them, and a stray",
    "# untracked dir (a scratch folder, a parallel agent's workspace) used to block every update. If a",
    "# pulled file ever collides with an untracked path, git refuses that pull itself and we fail cleanly.",
    'if [ -n "$(git status --porcelain --untracked-files=no 2>/dev/null)" ]; then',
    '  fail "local changes to tracked files present — the updater will not discard them; commit/stash or revert, then retry" "preparing"',
    "fi",
    "",
