...
export function normalizeDeviceName(value: unknown): string | undefined {
  ...
  if (!normalized || normalized.length > 80 || /[\p{Cc}\p{Zl}\p{Zp}]/u.test(normalized)) return undefined;
  return normalized;
}
...
function inMemoryStore(opts: OpenDeviceStoreOptions): DeviceStore {
  ...
  const pairings = new Map<
    string,
    { expiresAt: number; scopes: DeviceScope[]; deviceId?: string; tokenHash?: string }
  >();
