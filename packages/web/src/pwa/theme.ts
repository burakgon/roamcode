 ...
 * are simply off, so the app burns less battery and blacks read bottomless), or "light" (a paper-white
 * palette for bright-daylight use). A CLIENT-side preference (same localStorage convention as session
 * names): applied by setting `data-theme` on <html>, which tokens.css uses to override the surface
 * palette. Applied at boot (main.tsx, before first paint) and instantly from Settings.
 */
