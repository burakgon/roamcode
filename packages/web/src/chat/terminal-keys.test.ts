...
test("mode-independent keys emit fixed sequences", () => {
  ...
  expect(KEY_SEQUENCES.ShiftTab).toBe("\x1b[Z");
  expect(KEY_SEQUENCES.Delete).toBe("\x1b[3~");
  expect(KEY_SEQUENCES["|"]).toBe("|");
  expect(KEY_SEQUENCES["~"]).toBe("~");
});

test("cursor keys are CSI in normal mode and SS3 in application-cursor mode", () => {
...
test("keySequence routes cursor keys by mode and falls back to fixed/raw", () => {
  ...
  expect(keySequence("Esc", true)).toBe("\x1b");
  expect(keySequence("/", false)).toBe("/");
});

test("ctrl maps letters and useful punctuation to control bytes", () => {
...
test("persistent modifier locks encode text and Backspace", () => {
