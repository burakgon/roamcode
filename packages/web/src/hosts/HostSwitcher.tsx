...
export function HostSwitcher({
  ...
  return (
    <section className="rc-hosts" aria-label="RoamCode hosts">
      <div className="rc-hosts__bar">
        <label className="rc-hosts__select-label">
          <span className="sr-only">Active host</span>
          <select value={active.id} onChange={(event) => onActivate(event.target.value)} aria-label="Active host">
        ...
        {globalAttention > 0 && (
          <span className="rc-hosts__attention" role="status" aria-label={`${globalAttention} items across hosts`}>
            {globalAttention}
          </span>
        )}
        <button
      ...
      {manage && (
        <div className="rc-hosts__manager">
          <div className="rc-hosts__manager-head">
            <strong className="display">Hosts</strong>
          ...
          {adding && (
            ...
            >
              ...
              <small>The credential is stored only under this host entry and is never included in its URL.</small>
            ...
            <form
              ...
              onSubmit={(event) => {
                ...
                void onSearch(normalized)
                  ...
                  .catch(() => setError("Global search could not reach the hosts."))
                  .finally(() => setSearching(false));
              }}
            >
              <label>
                <span className="sr-only">Search all hosts</span>
                <input
                  type="search"
                  value={query}
                  onChange={(event) => setQuery(event.target.value)}
                  placeholder="Search all hosts"
                  maxLength={100}
                />
              </label>
          ...
          {registry.hosts.length > 1 && globalAttentionItems.length > 0 && (
            <section className="rc-hosts__global-attention" aria-labelledby="rc-global-attention-title">
              <strong id="rc-global-attention-title">Across hosts</strong>
              ...
              return (
                <li key={host.id} className="rc-hosts__row">
                  ...
                  <button
                    ...
                    disabled={registry.hosts.length === 1}
                    onClick={() => {
      ...
      <style>{`
        ...
        .rc-hosts select, .rc-hosts input { width: 100%; color: var(--text); background: var(--surface-2); border: 1px solid var(--border); border-radius: var(--radius-sm); padding: 8px; }
        .rc-hosts__attention { min-width: 22px; height: 22px; display: grid; place-items: center; border-radius: 999px; background: var(--coral); color: var(--bg); font-size: var(--fs-xs); font-weight: 700; }
        .rc-hosts__icon, .rc-hosts__text-button, .rc-hosts__remove, .rc-hosts__primary { min-height: 36px; border: 1px solid var(--border); border-radius: var(--radius-sm); background: var(--surface-2); color: var(--text); cursor: pointer; }
        .rc-hosts__icon { min-width: 36px; display: grid; place-items: center; }
        .rc-hosts__manager { display: grid; gap: var(--sp-2); margin-top: var(--sp-2); padding: var(--sp-3); border: 1px solid var(--border); border-radius: var(--radius); background: var(--glass-strong); }
        ...
        .rc-hosts__error { color: var(--danger); margin: 0; font-size: var(--fs-sm); }
        ...
        .rc-hosts__results button { width: 100%; display: grid; gap: 2px; padding: 8px; text-align: left; border: 1px solid var(--border); border-radius: var(--radius-sm); background: var(--surface-2); color: var(--text); cursor: pointer; }
        ...
        .rc-hosts__connect { display: flex; align-items: center; gap: 7px; border: 0; background: transparent; color: var(--text); text-align: left; cursor: pointer; min-width: 0; }
        ...
        .rc-hosts__dot--online { background: var(--success); }
        .rc-hosts__dot--revoked, .rc-hosts__dot--protocol-mismatch, .rc-hosts__dot--certificate-error { background: var(--coral); }
