...
- **A valid device key grants the installation's normal single-user access unless team role enforcement is enabled.**
  ...
  PWA, CLI, API, terminal sockets, and peer traffic. The host recovery key remains break-glass administration.
...
- **Peer federation is direct, explicit delegation rather than a sandbox.** The coordinating host stores a recoverable
  ...
  writer lease. HTTPS protects host-to-host traffic. See
  [the peer threat boundary](docs/peer-federation.md#security-and-failure-behavior).
